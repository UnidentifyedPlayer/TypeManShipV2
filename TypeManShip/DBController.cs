...
namespace TypeManShip
{
    public class DBController
    {
        ...
        private float[] SplitFloatString(string int_str)
        {
            ...
            for (int i = 0; i < arr.Length; i++)
            {
                float_arr[i] = Convert.ToSingle(arr[i], new CultureInfo("en-US", false));
            }
            return float_arr;
        }
        ...
        internal List<PhraseInputStat> GetUserData(int userid)
        {
            ...
            while (reader.Read())
            {
                ...
                object raw_vector = reader.GetValue(10);
                if(raw_vector != null)
                phrase_entry.bio_vecktor = SplitFloatString(Convert.ToString(raw_vector));
